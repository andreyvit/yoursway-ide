 ...
 * 
 * <ul>
 * <li>Never gc if there is a test harness present
 * <li>Don't gc if background jobs are running
 * <li>Don't gc if the keyboard or mouse have been active within IDLE_INTERVAL
 * <li>Don't gc if there has been a GC within the minimum gc interval (system
 * property PROP_GC_INTERVAL)
 * <li>After a gc, don't gc again until (duration * GC_DELAY_MULTIPLIER) has
 * elapsed. For example, if a GC takes 100ms and the multiplier is 60, don't gc
 * for at least five seconds
 * <li>Never gc again if any single gc takes longer than system property
 * PROP_GC_MAX
 * </ol>
 */
class IDEIdleHelper {
    
    private final static boolean DEBUG_GC = Boolean.valueOf(Platform
            .getDebugOption("com.yoursway.ide.rcp/debugBackgroundGarbageCollections"));
    ...
    private void createGarbageCollectionJob() {
        gcJob = new Job("Collecting garbage") {
