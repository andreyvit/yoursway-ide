...
public class IDEApplication implements IApplication, IExecutableExtension {
    ...
    private boolean checkInstanceLocation(Shell shell) {
        ...
        Location instanceLoc = Platform.getInstanceLocation();
        if (instanceLoc == null) {
            MessageDialog.openError(shell, "Workspace is Mandatory",
                    "IDEs need a valid workspace. Restart without the @none option.");
            return false;
        }
        ...
        if (instanceLoc.isSet()) {
            ...
            try {
                ...
                Activator.log("Could not obtain lock for workspace location", //$NON-NLS-1$
                        e);
        ...
        while (true) {
            ...
            MessageDialog.openError(shell, "Workspace Unavailable",
    ...
    private URL promptForWorkspace(Shell shell, ChooseWorkspaceData launchData, boolean force) {
        ...
        do {
            ...
            if (instancePath.length() <= 0) {
                MessageDialog.openError(shell, "Workspace Required",
                        "Workspace field must not be empty; enter a path to continue.");
                continue;
            }
            ...
            try {
                ...
                MessageDialog.openError(shell, "Invalid Workspace",
                        "Selected workspace is not valid; choose a different one.");
                continue;
            }
    ...
    private static void writeWorkspaceVersion() {
        ...
        try {
            ...
            Activator.log("Could not write version file", //$NON-NLS-1$
                    e);
        } finally {
            try {
                if (output != null) {
