...
public class CompletionProposalPopup {
    ...
    private void updateProposalList() {
        List<WorksheetCompletionProposal> proposals = console.getWorksheetCompletionProposals();
        ...
        if (proposalsChanged(proposals)) {
            ...
            for (WorksheetCompletionProposal proposal : proposals) {
    ...
    private boolean proposalsChanged(List<WorksheetCompletionProposal> newProposals) {
        if (proposalTable.getItemCount() != newProposals.size())
            return true;
        
        for (int i = 0; i < newProposals.size(); i++) {
            WorksheetCompletionProposal proposal = (WorksheetCompletionProposal) proposalTable.getItem(i).getData();
    ...
    private void apply(TableItem tableItem, boolean select) {
        WorksheetCompletionProposal proposal = (WorksheetCompletionProposal) tableItem.getData();
        console.useWorksheetCompletionProposal(proposal, select);
    }
