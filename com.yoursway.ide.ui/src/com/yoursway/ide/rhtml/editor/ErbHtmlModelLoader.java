...
import com.yoursway.ide.rhtml.internal.modelquery.JSPModelQueryAdapterImpl;
...
public class ErbHtmlModelLoader extends AbstractModelLoader {
    ...
    private EmbeddedTypeHandler getJSPDefaultEmbeddedType(IStructuredModel model) {
        ...
        if (model != null) {
            ...
            if (file != null) {
                mimeType = JSPFContentProperties
                        .getProperty(JSPFContentProperties.JSPCONTENTTYPE, file, true);
            }
        }
...
//        result.add(factory);
     ...
     */
    private void reInitializeEmbeddedType(IStructuredModel model, EmbeddedTypeHandler oldEmbeddedContentType,
     ...
     * one assigned, before we start checking the stream
     */
    private void initCloneOfEmbeddedType(IStructuredModel model, EmbeddedTypeHandler oldEmbeddedContentType,
            EmbeddedTypeHandler newEmbeddedContentType) {
        // check program logic
        Assert.isNotNull(oldEmbeddedContentType, "Program error: invalid call during model initialization"); //$NON-NLS-1$
        // once we know the embedded content type, we need to set it in the
        ...
        if (embeddedContentType == null) {
            ...
            if (desc != null) {
                ...
                if (embeddedContentType == null) {
                    ...
                    if (prop != null) {
                        embeddedContentType = EmbeddedTypeRegistryImpl.getInstance()
                                .getTypeFor((String) prop);
                    }
                }
            }
        }
    ...
    public IStructuredModel createModel(IStructuredModel oldModel) {
        ...
        if (modelQueryAdapter == null) {
            ...
            document.addAdapter(new JSPModelQueryAdapterImpl(modelQueryAdapter.getCMDocumentCache(),
                    modelQueryAdapter.getModelQuery(), modelQueryAdapter.getIdResolver()));
