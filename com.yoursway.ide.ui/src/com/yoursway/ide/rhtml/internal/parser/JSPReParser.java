package com.yoursway.ide.rhtml.internal.parser;
...
public class JSPReParser extends XMLStructuredDocumentReParser {
    ...
    /**
     * If a comment start or end tag is being added or deleted, we'll rescan the
     * whole document. The reason is that content that is revealed or commented
     * out can effect the interpretation of the rest of the document. Note: for
     * now this is very XML/JSP specific, can refactor/improve later.
     */
    @Override
     ...
     * super's method. changes/fixes to super may have to be made here as well.
     */
    @Override
    protected StructuredDocumentEvent reparse(IStructuredDocumentRegion dirtyStart,
            IStructuredDocumentRegion dirtyEnd) {
        StructuredDocumentEvent result = null;
        int rescanStart = -1;
        int rescanEnd = -1;
        ...
        if (containsBreakingChange(oldNodes) || isBreakingWithNestedTag(dirtyStart, dirtyEnd)) {
            ...
            oldNodes = formOldNodes(fStructuredDocument.getFirstStructuredDocumentRegion(),
     ...
     * IStructuredDocumentRegion, contain regions that could alter the behavior
     * of the parser or the parsing of areas outside of the regions given.
     */
     ...
     * XML_TAG_OPEN that's in it's own IStructuredDocumentRegion and preceded by
     * an unended IStructuredDocumentRegion 2) ...a deletion happening in an
     * XML_EMPTY_TAG_CLOSE that ends a ITextRegionContainer 3) ...an insertion
     * happening with a ' <' character somewhere in an XML attribute name or
     * value 4) ...a deletion of a normal XML_TAG_CLOSE since subsequent tags
     * become attribute values
     */
    
    private boolean isBreakingWithNestedTag(boolean changesIncludeA_lt, boolean delsIncludeA_gt,
            IStructuredDocumentRegion parent, ITextRegion region) {
        boolean result = false;
        
        IStructuredDocumentRegion previous = parent.getPrevious();
        // case 1 test
        if (parent.getRegions().size() == 1
                && region.getType() == DOMRegionContext.XML_TAG_OPEN
                && (previous == null || (!previous.isEnded() || previous.getType() == DOMRegionContext.XML_CONTENT))) {
            result = true;
        }
        // case 2 test
        ...
        // case 3 test
        if (changesIncludeA_lt
                && (region.getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_NAME || region.getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_VALUE)) {
            result = true;
        }
        // case 4 test
    ...
    private boolean isJspRoot(ITextRegionList regions) {
        return regions.size() > 1 && regions.get(0).getType() == DOMRegionContext.XML_TAG_OPEN
                && regions.get(1).getType() == DOMJSPRegionContexts.JSP_ROOT_TAG_NAME;
    }
    ...
    private boolean isTaglibOrInclude(IStructuredDocumentRegion node, ITextRegionList regions) {
        boolean sizeAndTypesMatch = (regions.size() > 1)
                && (regions.get(1).getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_NAME)
                && (regions.get(0).getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_OPEN || regions.get(0)
...
//                && (directiveName.equals(JSP11TLDNames.TAGLIB) || directiveName.equals(JSP11TLDNames.INCLUDE)
//                        || directiveName.equals(JSP12Namespace.ElementName.DIRECTIVE_TAGLIB) || directiveName
//                        .equals(JSP12Namespace.ElementName.DIRECTIVE_INCLUDE));
    ...
    private boolean intersects(IStructuredDocumentRegion node, ITextRegion region, int low, int high) {
        ...
        return (end >= low && start <= high) || (start <= low && end >= low)
                || (start <= high && end >= high);
    }
    ...
    private boolean isJSPEmbeddedStartOrEnd(IStructuredDocumentRegion flatNode) {
        ...
        result = ((type == DOMJSPRegionContexts.JSP_SCRIPTLET_OPEN)
                || (type == DOMJSPRegionContexts.JSP_EXPRESSION_OPEN) || (type == DOMJSPRegionContexts.JSP_DECLARATION_OPEN));
        return result;
    }
    ...
    protected boolean isPartOfBlockRegion(IStructuredDocumentRegion flatNode) {
        boolean result = false;
        String type = flatNode.getType();
        result = ((type == DOMJSPRegionContexts.JSP_CLOSE) || (type == DOMJSPRegionContexts.JSP_CONTENT) || super
                .isPartOfBlockRegion(flatNode));
        return result;
    }
