...
import com.yoursway.ide.rhtml.internal.parser.internal.JSPTokenizer;
...
public class JSPSourceParser extends XMLSourceParser implements JSPCapableParser {
    protected class NestablePrefixHandler implements StructuredDocumentRegionHandler,
        ...
        public void nodeParsed(IStructuredDocumentRegion aCoreFlatNode) {
            ...
            if (aCoreFlatNode.getNumberOfRegions() > 4
                    && aCoreFlatNode.getRegions().get(1).getType() == DOMJSPRegionContexts.JSP_DIRECTIVE_NAME) {
...
//                        taglibdetected = ((IRegionComparible) fCharSequenceSource).regionMatches(offset,
//                                length, JSP12TLDNames.TAGLIB);
                        taglibdirectivedetected = ((IRegionComparible) fCharSequenceSource).regionMatches(
                                offset, length, JSP12Namespace.ElementName.DIRECTIVE_TAGLIB);
                    } else {
                        // old fashioned way
                        String directiveName = getText(offset, length);
//                        taglibdetected = directiveName.equals(JSP12TLDNames.TAGLIB);
                        taglibdirectivedetected = directiveName
                                .equals(JSP12Namespace.ElementName.DIRECTIVE_TAGLIB);
                    }
            ...
            else if (aCoreFlatNode.getNumberOfRegions() > 4
                    && (aCoreFlatNode.getRegions().get(1)).getType() == DOMJSPRegionContexts.JSP_ROOT_TAG_NAME) {
                ...
                for (int i = 0; i < regions.size(); i++) {
                    ...
                    if (region.getType() == DOMRegionContext.XML_TAG_ATTRIBUTE_NAME) {
                        if (fCharSequenceSource != null && fCharSequenceSource instanceof IRegionComparible) {
                            int offset = taglibFlatNode.getStartOffset(region);
                            int length = region.getTextLength();
//                            prefixname = ((IRegionComparible) fCharSequenceSource).regionMatches(offset,
//                                    length, JSP12TLDNames.PREFIX);
                        } else {
                            // old fashioned way
//                            prefixname = (getText(taglibFlatNode.getStartOffset(region), region
//                                    .getTextLength()).equals(JSP12TLDNames.PREFIX));
                        }
        ...
        public void setStructuredDocument(IStructuredDocument newDocument) {
            ...
            if (newDocument != null && newDocument.getParser() instanceof StructuredDocumentRegionParser) {
                ((StructuredDocumentRegionParser) newDocument.getParser())
                        .addStructuredDocumentRegionHandler(this);
            }
        }
    }
    ...
    protected IStructuredDocumentRegion parseNodes() {
        ...
        while ((region = getNextRegion()) != null) {
            ...
            // the following contexts OPEN new StructuredDocumentRegions
            else if ((currentNode != null && currentNode.isEnded()) || (type == DOMRegionContext.XML_CONTENT)
                    || (type == DOMRegionContext.XML_CHAR_REFERENCE)
                    || (type == DOMRegionContext.XML_ENTITY_REFERENCE)
                    || (type == DOMRegionContext.XML_PI_OPEN) || (type == DOMRegionContext.XML_TAG_OPEN)
                    ...
                    || (type == DOMJSPRegionContexts.JSP_COMMENT_OPEN)
            ...
            // cleanly
                    ...
                    || (type == DOMRegionContext.XML_CDATA_CLOSE) || (type == DOMJSPRegionContexts.JSP_CLOSE)
