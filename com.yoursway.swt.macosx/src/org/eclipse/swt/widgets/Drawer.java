...
import com.yoursway.swt.experiments.MacOS;
 ...
 */
public Drawer (int style) {
	this ((Display) null, style);
}

/**
 * Constructs a new instance of this class give only the display
 ...
 * @see SWT#SYSTEM_MODAL
 */
public Drawer (Display display, int style) {
	this (display, null, style, 0, false);
}

Drawer (Display display, Shell parent, int style, int handle, boolean embedded) {
	super ();
	checkSubclass ();
 ...
 * @see SWT#SYSTEM_MODAL
 */
public Drawer (Shell parent, int style) {
	this (parent != null ? parent.display : null, parent, style, 0, false);
}

public static Drawer internal_new (Display display, int handle) {
	return new Drawer (display, null, SWT.NO_TRIM, handle, false);
}
...
void fixShell (Drawer newShell, Control control) {
...
//	setVisible (true);
	...
	MacOS.SetDrawerParent(shellHandle, parent.getShell().shellHandle);
	MacOS.OpenDrawer(shellHandle, 0, false);
    state &= ~HIDDEN;
    windowMadeVisible();
	if (active) {
		if (!restoreFocus () && !traverseGroup (true)) setFocus ();
...
private void windowMadeVisible() {
    sendEvent (SWT.Show);
    if (isDisposed ()) return;
