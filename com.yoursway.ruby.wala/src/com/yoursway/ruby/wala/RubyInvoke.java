...
public class RubyInvoke extends AbstractLexicalInvoke {
    /**
    ...
    public RubyInvoke(int function, int result, int[] params, int exception, CallSiteReference site) {
        super(result, exception, site);
        this.function = function;
        this.params = params;
    }
    
    private RubyInvoke(int function, int result, int[] params, int exception, CallSiteReference site,
     ...
     * @param params
     */
    public RubyInvoke(int function, int[] params, int exception, CallSiteReference site) {
        ...
        if (uses != null) {
            ...
            if (lexicalReads != null) {
                ...
                for (int j = 0; j < reads.length; j++)
                    reads[j] = new Access(lexicalReads[j].variableName, lexicalReads[j].variableDefiner,
                            uses[i++]);
            }
        }
        ...
        if (defs != null) {
            ...
            if (lexicalWrites != null) {
                ...
                for (int j = 0; j < writes.length; j++)
                    writes[j] = new Access(lexicalWrites[j].variableName, lexicalWrites[j].variableDefiner,
                            defs[i++]);
            }
        }
        
        return new RubyInvoke(fn, newLval, newParams, newExp, site, reads, writes);
    }
        ...
        if (lexicalReads != null) {
            ...
            for (int i = 0; i < lexicalReads.length; i++) {
                s.append(" ").append(lexicalReads[i].variableName).append(":").append(
                        getValueString(symbolTable, d, lexicalReads[i].valueNumber));
            }
            s.append(")");
        }
        ...
        if (lexicalWrites != null) {
            ...
            for (int i = 0; i < lexicalWrites.length; i++) {
                s.append(" ").append(lexicalWrites[i].variableName).append(":").append(
                        getValueString(symbolTable, d, lexicalWrites[i].valueNumber));
            }
            s.append(")");
        }
    ...
    public void visit(IVisitor v) {
        if (!(v instanceof RubyInvokeVisitor))
            System.out.println();
        Assertions._assert(v instanceof RubyInvokeVisitor);
        ((RubyInvokeVisitor) v).visitRubyInvoke(this);
    }
