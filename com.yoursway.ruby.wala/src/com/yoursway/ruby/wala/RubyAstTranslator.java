...
public class RubyAstTranslator extends AstTranslator {
  private final static boolean DEBUG = true;

  public RubyAstTranslator(RubyClassLoader loader) {
    super(loader);
  }
				...
				DebuggingInformation debugInfo)
  {
    ...
    ((RubyClassLoader)loader).defineCodeBodyCode("L"+fnName, 
			      cfg, 
			      symtab, 
			      hasCatchBlock, 
			      caughtTypes,
  ...
  protected void doCall(WalkContext context, CAstNode call, int result, int exception, CAstNode name, int receiver, int[] arguments) {
	...
	RubyMethods.ctorReference:
	AstMethodReference.fnReference(RubyTypes.CodeBody);
    
    // FIXME: missing receiver (was a first arg)
    context.cfg().addInstruction(
      new SSAInvokeInstruction( result, arguments, exception, 
        new RubyCallSiteReference(ref, context.cfg().getCurrentInstruction())));
  ...
  protected void doMaterializeFunction(WalkContext context, int result, int exception, CAstEntity fn) {
    ...
    context.cfg().addInstruction(
      new SSAInvokeInstruction( result, new int[]{ nm }, exception,
        new RubyCallSiteReference(
          RubyMethods.ctorReference,
	  context.cfg().getCurrentInstruction())));
  }
...
//    }
  ...
  public void translate(CAstEntity N, String nm) {
      AdHocView.translatingEntity(N, nm);
