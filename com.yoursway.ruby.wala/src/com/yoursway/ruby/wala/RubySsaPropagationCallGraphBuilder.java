...
import com.yoursway.js.wala.JavaScriptTypeOfInstruction;

public class RubySsaPropagationCallGraphBuilder extends AstSSAPropagationCallGraphBuilder {
...
//  }
  ...
  protected ConstraintVisitor makeVisitor(ExplicitCallGraph.ExplicitNode node) {
    return new RubyConstraintVisitor(this, node);
  }

  public static class RubyConstraintVisitor extends AstConstraintVisitor implements RubyCustomIntructionsVisitor {

    public RubyConstraintVisitor(AstSSAPropagationCallGraphBuilder builder, ExplicitCallGraph.ExplicitNode node) {
      super(builder, node);
    }
    ...
    public void visitUnaryOp(SSAUnaryOpInstruction inst) {
      if (inst.getOpcode() == UnaryOpInstruction.Operator.NEG) {
        ...
        IClass bool = getClassHierarchy().lookupClass(RubyTypes.Boolean);
    ...
    public void visitTypeOf(JavaScriptTypeOfInstruction instruction) {
      ...
      IClass string = getClassHierarchy().lookupClass(RubyTypes.String);
    ...
    public void visitRubyInvoke(RubyInvoke instruction) {
        ...
        if (contentsAreInvariant(symbolTable, du, instruction.getFunction())) {
          ...
          for (int i = 0; i < ik.length; i++) {
            ...
            if (n != null) {
              processRubyCall(node, instruction, n, ik[i], consts, uniqueCatch);
            }
          }
        } else {
          system.newSideEffect(new RubyDispatchOperator(instruction, node, consts, uniqueCatch), F);
        }
    }
    ...
    class RubyDispatchOperator extends UnaryOperator {
      private final RubyInvoke call;
      ...
      public boolean equals(Object o) {
        return (o instanceof RubyDispatchOperator) && ((RubyDispatchOperator) o).node.equals(node)
            && ((RubyDispatchOperator) o).call.equals(call);
      }
      ...
      /**
         ...
         * @param node
         */
      RubyDispatchOperator(RubyInvoke call, ExplicitCallGraph.ExplicitNode node, InstanceKey[][] constParams,
        ...
        if (receivers.getValue() != null) {
          receivers.getValue().foreachExcluding(previousReceivers, new IntSetAction() {
            public void act(int ptr) {
              ...
              if (target != null) {
                processRubyCall(node, call, target, iKey, constParams, uniqueCatch);
    ...
    void processRubyCall(CGNode caller, RubyInvoke instruction, CGNode target, InstanceKey function,
    ...
    private void handleBinaryOp(final SSABinaryOpInstruction instruction, final CGNode node, final SymbolTable symbolTable,
      ...
      class BinaryOperator extends AbstractOperator {
        ...
        private boolean isStringConstant(InstanceKey k) {
          return (k instanceof ConstantKey) && k.getConcreteType().getReference().equals(RubyTypes.String);
        }
