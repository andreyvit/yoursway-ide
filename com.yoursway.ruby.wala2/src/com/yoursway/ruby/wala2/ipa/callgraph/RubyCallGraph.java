...
public class RubyCallGraph extends AstCallGraph {
	...
	protected CGNode makeFakeRootNode() {
		return findOrCreateNode(new RubyFakeRoot(cha, options,
				getAnalysisCache()), Everywhere.EVERYWHERE);
	}

	@Override
	protected CGNode makeFakeWorldClinitNode() {
		return findOrCreateNode(new RubyFakeWorldClinitMethod(cha, options,
				getAnalysisCache()), Everywhere.EVERYWHERE);
	}
